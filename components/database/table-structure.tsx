...
export function TableStructure({
    ...
    return (
        <div className={cn('flex flex-col h-full bg-neutral-950/50', className)}>
            {/* Hero Header */}
            <div className="relative p-6 border-b border-neutral-800 bg-gradient-to-br from-neutral-900 via-neutral-900 to-neutral-900/50">
                <div className="absolute top-0 right-0 p-6 opacity-10 pointer-events-none">
                    <Database size={120} />
                </div>

                <div className="relative z-10 flex items-start gap-4">
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-emerald-900/20 border border-emerald-500/30 flex items-center justify-center shadow-lg shadow-emerald-900/20">
                        <Fingerprint size={28} className="text-emerald-400" />
                    </div>
                    <div>
                        <h1 className="text-2xl font-bold text-white tracking-tight">{tableName}</h1>
                        <p className="text-sm text-neutral-400 mt-1 max-w-2xl">{table.description || 'No description provided.'}</p>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        {columns.map(([name, col], idx) => (
                            <motion.div
                                ...
                                className="group relative flex items-center gap-4 p-3 rounded-xl bg-neutral-900/40 border border-neutral-800/60 hover:bg-neutral-800/40 hover:border-emerald-500/30 transition-colors duration-200"
                            >
                                {/* Index */}
